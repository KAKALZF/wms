...
public class ParametersInterceptor extends MethodFilterInterceptor {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(ParametersInterceptor.class);
			...
			if (parameters != null) {
				...
				try {
					...
					ReflectionContextState.setDenyMethodExecution(contextMap,
							true);
					ReflectionContextState.setReportingConversionErrors(
							contextMap, true);
	...
	protected void setParameters(final Object action, ValueStack stack,
		...
		if (memberAccessStack) {
			...
			accessValueStack.setAcceptProperties(acceptedPatterns
					.getAcceptedPatterns());
			accessValueStack.setExcludeProperties(excludedPatterns
					.getExcludedPatterns());
		}
	...
	protected void notifyDeveloperParameterException(Object action,
		...
		String developerNotification = LocalizedTextUtil.findText(
				...
				new Object[] { "Unexpected Exception caught setting '"
						+ property + "' on '" + action.getClass() + ": "
						+ message });
		LOG.error(developerNotification);
		// see https://issues.apache.org/jira/browse/WW-4066
		if (action instanceof ValidationAware) {
			Collection<String> messages = ((ValidationAware) action)
					.getActionMessages();
	...
	protected boolean acceptableName(String name) {
		...
		if (devMode && accepted) { // notify only when in devMode
			LOG.debug(
					"Parameter [#0] was accepted and will be appended to action!",
					name);
		}
		return accepted;
	}
	...
	protected boolean isWithinLengthLimit(String name) {
		...
		if (!matchLength) {
			notifyDeveloper(
					"Parameter [#0] is too long, allowed length is [#1]", name,
	...
	protected boolean isAccepted(String paramName) {
		AcceptedPatternsChecker.IsAccepted result = acceptedPatterns
				.isAccepted(paramName);
	...
	protected boolean isExcluded(String paramName) {
		ExcludedPatternsChecker.IsExcluded result = excludedPatterns
